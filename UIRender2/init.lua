...
-- style table (for simulate css style properties edit)
local contentsClass = {}; do
    contentsClass.__index = {};

    function contentsClass:put(thing)
        -- add item to stoarge
        table.insert(self,thing);
        for i,v in pairs(self.__style) do
            thing[i] = v;
        end
    end

    function contentsClass:pop(thing)
    ...
    function contentClass:setStyle(style)
        -- merge table
        local selfStyle = self.__style;
        for i,v in pairs(style) do
            selfStyle[i] = v;
        end

        -- set objects properties
        for _,thing in ipairs(self) do
            for i,v in pairs(style) do
                thing[i] = v;
            end
        end
    ...
    function contentsClass.new()
        local this = {
            __style = {};
        };

        setmetatable(this,contentsClass);

        return this;
    end
end

local renderClass = {}; do
    ...
    --[[
        ...
        class:renderClass
            funciton:draw
                discription
                    drow ui object to screen, if thing are given as table, drows things to one screenGui
                    or thing is given as gui item, then it moved into players gui

                arg:1:thing
                    input for render what you want to draw on screen

    ]]
    renderClass.event = setmetatable({},{
        __index = function (index)
            return "EV" .. index;
        end;
        __newindex = function (index,value)
            error("you can not set events from renderClass.event; these class have get function only! try to use renderClass.event.EVENTNAME");
        end;
    })
    ...
    function renderClass.new(settings)
        ...
        this.debug = settings.debug or warn or print; ---@diagnostic disable-line
